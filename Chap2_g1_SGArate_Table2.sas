...
;
RUN;
PROC FREQ DATA = DT.FINALDATA_ANL;
TABLES SAM_BIRPLMOM SAM_BIRPLDAD;
RUN;
DATA CHAP1A;
...
/*Just to create PTB and non-PTB*/
	IF 20 < GEST_WK < 38 THEN PTB_CAT = 1;
	ELSE IF 38 <= GEST_WK <=47 THEN PTB_CAT = 0;
...
IF SGA_wfg1 in (2, 5, 8) THEN SGA_WNHFG1 = 1;/*SGA*/
ELSE IF SGA_wfg1 in (1, 3, 4, 6, 7, 9) THEN SGA_WNHFG1 = 0; /*ACCEPTABLE (APPROPRIATE) */
...
TABLES SGA_WNHFG1 SGA_BNHFG1 SGA_NHFG1;
RUN;

/*Table 2 -SGA rates by RACE & ETHNICITY*/
PROC FREQ DATA = livefnh1;
TABLES SGA_WNHFG1* (SAM_Kessner SAM_meduc SAM_Feduc)/trend measures cl;
TITLE "SGA AMONG WNH delivering between 1979-1983";
FORMAT RACEHISP fmrace. SAM_LBW fy1n0x.;
RUN;
PROC FREQ DATA = livefnh1;
TABLES SGA_WNHFG1* (SAM_LBW SAM_MAGE3 SAM_MARITAL)/chisq cl;
WHERE RACEHISP EQ 0;
TITLE "SGA AMONG WNH delivering between 1979-1983";
FORMAT RACEHISP fmrace. SAM_LBW fy1n0x.;
RUN;
...
PROC FREQ DATA = livefnh1;
TABLES SGA_BNHFG1* (SAM_Kessner SAM_meduc SAM_Feduc)/trend measures cl;
TITLE "Grandmothers Low birthweight by race among women delivering between 1979-1983";
FORMAT RACEHISP fmrace. SAM_LBW fy1n0x.;
RUN;
PROC FREQ DATA = livefnh1;
TABLES SGA_BNHFG1* (SAM_LBW SAM_MAGE3 SAM_MARITAL)/chisq cl;
TITLE "Grandmothers Low birthweight by race among women delivering between 1979-1983";
FORMAT RACEHISP fmrace. SAM_LBW fy1n0x.;
RUN;
/*tABLE 2 H*/
...
PROC FREQ DATA = livefnh1;
TABLES SGA_NHFG1*(SAM_Kessner SAM_meduc SAM_Feduc)/trend measures cl;
TITLE "Grandmothers Low birthweight by race among women delivering between 1979-1983";
FORMAT RACEHISP fmrace. SAM_LBW fy1n0x.;
RUN;
PROC FREQ DATA = livefnh1;
TABLES SGA_NHFG1* (SAM_LBW SAM_MAGE3 SAM_MARITAL)/chisq cl;
TITLE "Grandmothers Low birthweight by race among women delivering between 1979-1983";
FORMAT RACEHISP fmrace. SAM_LBW fy1n0x.;
RUN;
...
/*look at the col percent*/

PROC FREQ DATA = livefnh1;
TABLES SGA_WNHFG1 * (SAM_Kessner SAM_meduc3 SAM_Feduc3)/trend measures cl;
RUN;
PROC FREQ DATA = livefnh1;
TABLES SGA_WNHFG1 * (SAM_LBW SAM_MAGE3 SAM_MARITAL)/CHISQ ;
RUN;
PROC FREQ DATA = livefnh1;
TABLES SGA_BNHFG1 * (SAM_Kessner SAM_meduc3 SAM_Feduc3)/trend measures cl;
RUN;
PROC FREQ DATA = livefnh1;
TABLES SGA_BNHFG1 * (SAM_LBW SAM_MAGE3 SAM_MARITAL)/CHISQ ;
RUN;
PROC FREQ DATA = livefnh1;
TABLES SGA_NHFG1 * (SAM_Kessner SAM_meduc3 SAM_Feduc3)/trend measures cl;
RUN;
PROC FREQ DATA = livefnh1;
TABLES SGA_NHFG1 * (SAM_LBW SAM_MAGE3 SAM_MARITAL)/CHISQ ;
RUN;

/*we do freq by SGA_WNHFG1*mat_meduc3, since we want col percent that tell the rate of the mothers that
preterm that had certain education level; if we do it other way we need to look at row percent

PROC FREQ DATA = livefnh1;
TABLES SGA_WNHFG1*SAM_meduc /trend measures cl;; *Col percent;
TABLES SAM_meduc6*PTB_CAT; *row percent;
WHERE RACEHISP EQ 0;
run;*/
...
ODS TAGSETS.EXCELXP
file='C:\Users\njain\Documents\Personal\Dissertation\Diss_workingfolder\Analysis\SGA\Chapter 2 -SGA\Output\MEANBWT_G1.xls'
STYLE=minimal
...
PROC means DATA = livefnh1;
class GEST_WK racehisp;
var BIRTHWT_F;
RUN;

...
PROC FREQ DATA = livefnh1;
TABLES PTB_CAT*SAM_meduc /trend measures cl;; *Col percent;
TABLES SAM_meduc6*PTB_CAT; *row percent;
WHERE RACEHISP EQ 0;
run;
/*Table 2 -Preterm rates by RACE & ETHNICITY*/
PROC FREQ DATA = livefnh1;
TABLES PTB_CAT*
PROC FREQ DATA = livefnh1;
TABLES PTB_CAT* (SAM_Kessner SAM_meduc SAM_Feduc)/trend measures cl;
WHERE RACEHISP EQ 0;
TITLE "Grandmothers Low birthweight by race among women delivering between 1979-1983";
FORMAT RACEHISP fmrace. SAM_LBW fy1n0x.;
RUN;
PROC FREQ DATA = livefnh1;
TABLES PTB_CAT* (SAM_LBW SAM_MAGE3 SAM_MARITAL)/chisq cl;
WHERE RACEHISP EQ 0;
TITLE "Grandmothers Low birthweight by race among women delivering between 1979-1983";
FORMAT RACEHISP fmrace. SAM_LBW fy1n0x.;
RUN;
...
/*SGA*/
PROC FREQ DATA = livefnh1;
TABLES (SGA_WNHFG1 SGA_BNHFG1 SGA_NHFG1)*RACEHISP;
RUN;

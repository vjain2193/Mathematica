...
SAM_meduc3 = .;
  ...
  else if '12' < SAM_EDUMOM < '18' then SAM_meduc3 = 2;
  else SAM_meduc3 = 3;

 SAM_feduc3 = .;
  IF 0 <= SAM_EDUDAD < 12 THEN SAM_Feduc3 = 1;
  else if SAM_EDUDAD eq '12' THEN SAM_Feduc3 = 2; 
  else if '12' < SAM_EDUDAD < '18' then SAM_Feduc3 = 3;
  else SAM_Feduc3 = 4;
...
SAM_Feduc = .;
  ...
  else IF SAM_EDUDAD = '&&' OR SAM_EDUDAD = 18 then SAM_Feduc = 6;
...
TABLES SGA_WNHF_G1*SAM_meduc /trend measures cl;; *Col percent;
WHERE RACEHISP EQ 0;
run;
/*************Regression analysis****************/
...
MODEL SGA_WNHF_G1 =  SAM_meduc RACEHISP;
RUN; 
PROC LOGISTIC DATA = BIN_REG descending;
class SAM_meduc (param=ref ref="3")
	  SAM_MAGE3(param=ref ref="3"); 	  
MODEL SGA_WNHF_G1 = SAM_meduc SAM_MAGE3;
RUN; 
PROC LOGISTIC DATA = BIN_REG descending;
class SAM_meduc (param=ref ref="3")
	  SAM_Feduc param=ref ref="3");
MODEL SGA_WNHF_G1 =  SAM_meduc SAM_Feduc;
RUN; 
PROC LOGISTIC DATA = BIN_REG descending;
class  SAM_meduc (param=ref ref="3");
MODEL SGA_WNHF_G1 = SAM_meduc SAM_married;
RUN; 
PROC LOGISTIC DATA = BIN_REG descending;
class SAM_meduc (param=ref ref="3")
	  SAM_Kessner(param=ref ref="1"); 	  
MODEL SGA_WNHF_G1 = SAM_meduc SAM_Kessner;
RUN; 
PROC LOGISTIC DATA = BIN_REG descending;
class SAM_meduc (param=ref ref="3");
MODEL SGA_WNHF_G1 = SAM_meduc SAM_LBW;
RUN; 
/*****STEPWISE********/
...
class RACEHISP (param=ref ref="0")
	 ...
	 RACEHISP(param=ref ref="0");
MODEL SGA_WNHF_G1 = SAM_meduc RACEHISP SAM_MAGE3;
RUN; 
...
class RACEHISP (param=ref ref="0")
	  ...
	  RACEHISP(param=ref ref="0")
	  SAM_Kessner (param=ref ref="1");
MODEL SGA_WNHF_G1 = SAM_meduc RACEHISP SAM_MAGE3 SAM_married ;
RUN; 
...
class RACEHISP (param=ref ref="0")
	  ...
	  SAM_Feduc (param=ref ref="4")
	  SAM_Kessner (param=ref ref="1");
MODEL SGA_WNHF_G1 = SAM_meduc RACEHISP SAM_MAGE3 SAM_married SAM_Feduc;
RUN; 
/*Final Model**/
PROC LOGISTIC DATA = BIN_REG descending;
class RACEHISP (param=ref ref="0")
	  ...
	  SAM_Feduc (param=ref ref="4")
	  SAM_Kessner (param=ref ref="1");
MODEL SGA_WNHF_G1 = SAM_meduc RACEHISP SAM_MAGE3 SAM_married SAM_Feduc SAM_Kessner;
RUN; 
